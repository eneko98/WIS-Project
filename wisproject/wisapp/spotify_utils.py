    ...
    for name in artist_names:
        ...
        if artist_search['artists']['items']:
            ...
            if artist_albums['items']:
                ...
                album_tracks = spotify.album_tracks(recent_album['id'], limit=1)  # Fetch only the most recent track
                if album_tracks['items']:
                    track = album_tracks['items'][0]
                    releases.append({
                        'name': track['name'],
                        'artist': name,
                        'release_date': recent_album['release_date'],  # Using the album release date
